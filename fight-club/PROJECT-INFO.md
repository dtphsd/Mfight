...
The project is a single frontend app in this `fight-club/` folder.

The parent `Project/` directory is only a multi-project container and should not be treated as the Fight Club root anymore.

The app root now also contains a local planning workflow:

- `MASTER-PLAN.md` for active tasks and sprint history
- `features/` for per-feature tracking documents
- `features/combat-design-reference.md` for combat-system documentation and safety tasks
- `features/hunting-mvp.md` for the planned autonomous hunting module and implementation roadmap
- `docs/README.md` as the GitBook-style documentation landing page
- `docs/SUMMARY.md` as the GitBook navigation tree
- `docs/gitbook-publish-setup.md` as the operational setup guide for GitBook publishing
- `docs/.vitepress/` and `docs/index.md` as the static docs-site layer for VitePress and GitHub Pages
...
- `docs/architecture/combat-design-reference.md` as the current source-of-truth combat runtime reference
- `docs/architecture/hunting-mvp-blueprint.md` as the original architecture blueprint for the autonomous hunting module
- `docs/architecture/hunting-runtime-reference.md` as the live hunting runtime reference and verification guide
- `Art/Avatars/` as the raw character-art source folder for combat silhouettes
- `BazaBK/` as the local Battle Kings HTML/image/catalog workspace
...
- `server/` hosts the online-duel HTTP authority runtime
...
The main menu now also exposes an `Admin Dashboard`:

- live PvP backend health visibility
- current frontend origin and configured backend URL visibility
- local operator notes for current frontend/backend tunnel URLs
- launch checklist for first remote PvP smoke tests
- launcher manifest sync from `online-duel-runtime.json`
- backend tunnel override support for browser-local PvP rescue and quick remote launch flow

The app root now also contains one-click PvP remote-play helpers:
...
- `runtime-logs/` for launcher output, tunnel logs, and quick-tunnel summaries
- `project-analysis/` for the local audit prompt and helper scripts
...
- launcher-assisted quick-tunnel bootstrap for first real remote matches
- browser-readable runtime manifest for operator handoff
...
- dedicated live PvP validation now also exists through:
  - `npm run test:pvp`
